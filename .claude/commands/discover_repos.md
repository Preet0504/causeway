...
Ask, in plain text, along these lines: "What kind of repository are you looking for? For example: 'popular Java web frameworks that are still actively maintained', 'small, quiet C libraries, not forks', or 'JSON parsing libraries with an MIT license'." This has to stay a free-text question, requirements are too varied for a small multiple-choice menu, but showing a few complete example answers gives the person a concrete sense of the expected shape of a reply, so include at least two or three phrased as full example answers, not a bare list of category names like "language, stars, activity, size". Don't try to map their answer onto specific flags yourself, that's the `repo-discovery` agent's job below, pass along what they actually said.
...
Read each `QUALIFY owner=... repo=... qualifies=true|false stars=... language=... sizeKb=... closedIssues=... openIssues=... mergedPrs=... openPrs=... testFiles=... reason=...` line, this is the only source you need for the table below, don't rely on the agent's own report for any of these fields.

## 4. Show the results

Render a markdown table of what was found: owner/repo, stars, language, size (the `sizeKb=...` reading from step 3, formatted human-readably: KB under 1024, otherwise MB to one decimal place), closed/open issues, merged/open PRs, test files, qualifies (yes/no), and, for anything that didn't qualify, the reason why, using step 3's qualification results directly, don't re-run anything to get this. State plainly which search flags were actually used (the agent reported this in step 2), so the person can tell whether their requirements were translated the way they expected.
